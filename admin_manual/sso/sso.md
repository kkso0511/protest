...
DirectCloud에서는 IdP와 SSO 연동을 통해, IdP가 제공하는 인증 정보를 사용하여 DirectCloud에 싱글 사인온할 수 있습니다.\
이 매뉴얼에서는, DirectCloud의 Web 매뉴얼에 기재된 SSO 연동과 관련된 용어의 정의와, DirectCloud와 IdP 간 연동 시의 사양, SAML 연동 시의 주의 사항 등에 대해 설명합니다.
...
이 기사에서는 다음 항목에 대해 설명합니다.\
클릭하면 각 제목으로 이동합니다.

* SSO 관련 용어
* DirectCloud에서 동작하는 IdP에 대해서
* 사용자 정보의 제휴와 갱신에 대해
* 액세스 제어에 대해
* 로그인과 로그아웃에 대해서
* Open API의 이용에 대해
...
**\[DirectCloud 계약 플랜에서 협력할 수 있는 IdP]**
...
IdP 측에서 관리하고 있는 사용자의 어떤 속성이 DirectCloud의 사용자 ID로 사용되는지에 대해서는, IdP의 속성과 DirectCloud의 사용자 정보와의 대조를 참조해 주십시오.\
...
* SSO 연동 후에 '사용자' > '계정 관리' 메뉴에서 IdP와 연동된 사용자 정보의 '이름'을 변경한 경우, IdP 측에 등록된 사용자 정보와 차이가 발생할 수 있습니다.
* DirectCloud의 '사용자 목록 가져오기' 기능을 지원하는 IdP의 경우, IdP의 사용자 정보 '가져오기'를 실행한 시점에 사용자 정보가 반영됩니다.



**\[DirectCloud의 '이름'에 연결되는 IdP의 속성에 대해]**

SSO 연동 시 DirectCloud의 '이름'에 연결되는 정보는 IdP에 따라 다릅니다.\
자세한 내용은 IdP의 속성과 DirectCloud의 사용자 정보와의 대조를 참조해 주십시오.

*   아래 IdP의 경우에는 '성', '이름' 순서가 아니라 '이름', '성' 순서로 연결된다는 점에 주의가 필요합니다. 예를 들어, 아래 IdP와 SSO 연동하여 DirectCloud에 SSO 로그인하면, 왼쪽 상단에 '신바시 이치로'가 아니라 '이치로 신바시'와 같이 표시됩니다.

    * Google Workspace
    * CloudGate UNO
    * Okta

    또한 아래 IdP의 경우에는 이메일 주소 형식의 사용자명이 DirectCloud의 '이름'에 연결되므로, '성', '이름' 정보는 DirectCloud의 '이름'에 연결되지 않습니다.\
    IdP의 '성', '이름'과 DirectCloud의 '이름' 정보를 일치시키려면, 사용자 정보를 변경하는 방법을 통해 수동으로 변경해야 합니다.

    * ID Entrance
* 아래 IdP의 경우에는 DirectCloud의 '이름'에 연결되는 '성'과 '이름' 사이에 반각 공백이 추가됩니다.
  * OneLogin
  * Sateraito Office / NextSet
  * Okta
...
|                            | IdP 속성       | DirectCloud 사용자 정보             |    |        |   |
| -------------------------- | ------------ | ------------------------------ | -- | ------ | - |
| 관리 화면 표시                   | 사용자 연결 작업시   | 사용자 ID                         | 이름 | 이메일 주소 |   |
| Microsoft Entra ID         | 이름           | -                              |    | ◯      |   |
| 사용자 프린시펄 이름                | -            | ◯                              |    | ◯      |   |
| ADFS                       | 표시 이름        | displayname                    |    | ◯      |   |
| UserPrincipalName          | upn          | ◯                              |    |        |   |
| 이메일 주소                     | emailaddress |                                |    | 〇      |   |
| CloudGate UNO              | 표시 이름        | GivenName + SurName            |    | ◯      |   |
| 사용자 ID                     | EmailAddress | ◯                              |    | ◯      |   |
| Google Workspace           | 이름           | firstname + lastname           |    | ◯      |   |
| 메일                         | Email        | ◯                              |    | ◯      |   |
| OneLogin                   | 성 + 이름       | lastname + firstname           |    | ◯      |   |
| 이메일 주소                     | Email        | ◯                              |    | ◯      |   |
| Sateraito Office / NextSet | 성 + 이름       | lastname + firstname           |    | ◯      |   |
| 이메일 주소                     | email        | ◯                              |    | ◯      |   |
| HENNGE One                 | 표시 이름        | {user.display\_name}           |    | ◯      |   |
| UserPrincipalName          | {user.upn}   | ◯                              |    |        |   |
| 이메일 주소                     | {user.email} |                                |    | ◯      |   |
| Salesforce                 | 성명           | -                              |    | ◯      |   |
| 사용자 이름                     | -            | ◯                              |    | ◯      |   |
| Okta                       | 성 + 이름       | user.firstName + user.lastName |    | ◯      |   |
| 이메일 주소                     | user.email   | ◯                              |    | ◯      |   |
| TrustLogin                 | 이름           | 멤버 성 + 멤버 이름                   |    | ◯      |   |
| 회원 이메일 주소                  | 회원 이메일 주소    | ◯                              |    | ◯      |   |
| OPTiM ID+                  | 이름           | 이름                             |    | ◯      |   |
| 이메일 주소                     | 이메일 주소       | ◯                              |    | ◯      |   |
| IIJ ID 서비스                 | 성명           | 성 + 이름                         |    | ◯      |   |
| ID                         | 알림 대상 이메일 주소 | ◯                              |    | ◯      |   |
| Soliton OneGate            | 로그인 이름       | id                             | ◯  |        |   |
| SSO User                   | NameID       |                                | ◯  |        |   |
| 이메일 주소                     | email        |                                |    | ◯      |   |
| Qualitia CLOUD             | 로그인 ID       | LOGIN\_ID                      |    |        |   |
| 이름                         | NAME         |                                |    |        |   |
| 이메일 주소                     | EXTERNAL\_ID | ◯                              | ◯  | ◯      |   |
| PassLogic                  | 성명           | 성명                             |    | ◯      |   |
| 이메일 주소                     | 이메일 주소       | ◯                              |    | ◯      |   |
| ID Entrance                | 사용자 이름       | username                       | ◯  | ◯      | ◯ |
