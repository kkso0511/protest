---
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/FRc8hBWqHCHYh3E0AhFb/admin_manual/file_share/upload_request_password
---

# 업로드 요청 화면에서 비밀번호 설정을 허용 또는 비허용으로 설정하는 방법

### 개요 <a href="#a03" id="a03"></a>

관리자가 업로드 요청의 비밀번호를 강제로 적용하지 않고, 사용자 페이지의 업로드 요청 생성 화면에서 설정할 수 있도록 하는 것도 가능합니다.\
이 매뉴얼에서는 ‘업로드 요청 비밀번호 설정’에서 업로드 요청의 비밀번호를 사용자 측에서 변경할 수 있도록 설정하는 방법에 대해 설명합니다.

***

### 제한·보충사항 <a href="#a04" id="a04"></a>

* 관리자 권한이 부여된 사용자는 관리자 페이지에서 '보안 정책'에 접근 가능합니다.
* ‘링크 옵션 설정’은 Guest, LDAP 연동 및 SAML 연동으로 등록된 사용자에게도 적용되지만, Connect User에는 적용되지 않습니다.
* '비밀번호 정책'의 설정 내용은 업로드 요청 폴더에는 적용되지 않습니다.

***

### 절차 <a href="#a05" id="a05"></a>

**\[관리자 페이지에서의 설정]**

1. '보안 정책' > '반출 정책' 메뉴를 선택합니다.

<figure><img src="../../.gitbook/assets/image (1660).png" alt=""><figcaption></figcaption></figure>



2. '업로드 요청 이메일 발송 시 비밀번호 옵션'의 '비밀번호'에서 '사용안함'을 선택합니다.

<figure><img src="../../.gitbook/assets/image (2514).png" alt=""><figcaption></figcaption></figure>



3. '저장' 버튼을 클릭합니다.



**\[사용자 페이지에서 작업]**

1. 업로드 요청 폴더 링크 보내기 화면에서 다음과 같이 받는 폴더의 비밀번호를 사용하도록 설정할 수 있는지 확인합니다.

<figure><img src="../../.gitbook/assets/image (1707).png" alt=""><figcaption></figcaption></figure>
